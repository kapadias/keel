# 2026-09-23 — Nonna vs a bare agent

The number a landing page owes its reader: the same model on the same tasks with **no harness at
all** versus Nonna. The earlier evals ([v1](2026-09-22-ladder.md), [v2](2026-09-22-ladder-v2.md))
...
you finish" replaced by "Review your change before you finish"; `nonna` = the harness with
`/review` forced. Claude Sonnet, headless, two runs per task per arm (n = 12 per arm). The
`nonna` rows are the v2 `after` runs reused unchanged, from the archived commit `04f0772`; only
...
[`2026-09-23-bare-vs-nonna-runs.tsv`](2026-09-23-bare-vs-nonna-runs.tsv).
...
| **Nonna**       |   12/12 |       12/12 |                0 |           16.5 (21.7) |     $2.82 |     429 s |
...
- **Nonna's cost is the loop, not the ladder.** Twenty times the money and fifteen times the
...
- **So the pitch cannot be "less code."** It is: every change ships with proof. Whether that is
  ...
  throwaway script, no — and Nonna's `/fix` fast lane exists for the middle.
...
modes Nonna's gates are for. Score "did it get caught," not lines.
