...
# A glob is a pattern, not a name: brackets, braces and ? pick secret files as well as * does.
printf '%s' '{"tool_name":"Grep","tool_input":{"pattern":"x","path":".","glob":"*.pe[m]"}}' | "$SS" 2>/dev/null; check "blocks a Grep glob with a bracket" 2 "$?"
printf '%s' '{"tool_name":"Grep","tool_input":{"pattern":"x","path":".","glob":"{*.pem,x}"}}' | "$SS" 2>/dev/null; check "blocks a Grep glob with braces" 2 "$?"
printf '%s' '{"tool_name":"Grep","tool_input":{"pattern":"x","path":".","glob":".e[n]v"}}' | "$SS" 2>/dev/null; check "blocks a bracketed .env glob" 2 "$?"
printf '%s' '{"tool_name":"Grep","tool_input":{"pattern":"x","path":".","glob":"*.[k]ey"}}' | "$SS" 2>/dev/null; check "blocks a bracketed .key glob" 2 "$?"
printf '%s' '{"tool_name":"Grep","tool_input":{"pattern":"x","path":".","glob":".e?v"}}' | "$SS" 2>/dev/null; check "blocks a ? in a .env glob" 2 "$?"
printf '%s' '{"tool_name":"Grep","tool_input":{"pattern":"x","path":".","glob":"*.PEM"}}' | "$SS" 2>/dev/null; check "blocks an upper-case extension glob" 2 "$?"
printf '%s' '{"tool_name":"Grep","tool_input":{"pattern":"x","path":".","glob":"*.{ts,tsx}"}}' | "$SS"; check "allows an ordinary brace glob" 0 "$?"
printf '%s' '{"tool_name":"Grep","tool_input":{"pattern":"x","path":".","glob":"!*.pem"}}' | "$SS"; check "allows a glob that excludes key files" 0 "$?"
