...
elif [ -n "$keel_root" ]; then
  keel_link="${keel_root}/hooks/require-status-sync.sh"
fi
...
# 4. Plugin install: carry the constitution in (keel_core_carrier, lib/core.sh —
...
keel_emit_context SessionStart "$msg"
exit 0
