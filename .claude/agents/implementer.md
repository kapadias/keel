...
- A **MEDIUM or LOW** review ask that adds code with no failing input named — `check-review.sh`
  prints these as `optional:` — gets a `debt:` marker
  with its trigger and a line under Remaining risk — not the code. Never a CRITICAL/HIGH finding,
  trust-boundary validation, or a fail-closed path: those are built. The ladder sizes the fix as
